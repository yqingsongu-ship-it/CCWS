<template>
  <div class="api-detail-page">
    <a-page-header title="API 监控详情" :ghost="false" @back="() => $router.push('/apis')">
      <template #extra>
        <a-space>
    ...
    <a-row :gutter="16" class="mt-4">
      ...
      <a-col :span="8">
        <a-card title="基本信息" :bordered="false">
          <a-descriptions :column="1" size="small">
            ...
            <a-descriptions-item label="请求方法"><a-tag :color="getMethodColor(monitor?.apiConfig?.method)">{{ monitor?.apiConfig?.method || 'GET' }}</a-tag></a-descriptions-item>
      ...
      <!-- 断言配置 -->
      <a-col :span="8">
        <a-card title="断言配置" :bordered="false">
          <a-descriptions :column="1" size="small">
            <a-descriptions-item label="断言数量">{{ monitor?.apiConfig?.assertions?.length || 0 }}</a-descriptions-item>
            <a-descriptions-item label="断言类型">
              <a-space wrap>
                <a-tag v-for="(assertion, idx) in monitor?.apiConfig?.assertions" :key="idx" color="blue">{{ assertion.type }}</a-tag>
                <span v-if="!monitor?.apiConfig?.assertions || monitor.apiConfig.assertions.length === 0">-</span>
              </a-space>
            </a-descriptions-item>
          </a-descriptions>
        </a-card>
      ...
      <a-table :columns="checkResultColumns" :data-source="checkResults" :loading="checkResultsLoading" :pagination="checkResultPagination" @change="handleCheckResultTableChange" size="small">
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'error'">
            <a-typography-text v-if="record.errorMessage" :ellipsis="{ tooltip: record.errorMessage }" type="danger">{{ record.errorMessage }}</a-typography-text>
            <span v-else>-</span>
          </template>
        </template>
      </a-table>
    ...
    <a-card title="告警历史" class="mt-4" :bordered="false">
      ...
      <a-timeline v-else>
        <a-timeline-item v-for="alert in alerts" :key="alert.id" :color="getAlertColor(alert.severity)">
          <template #dot><a-badge :status="getAlertBadgeStatus(alert.severity)" /></template>
...
const stats = ref({ uptime: 0, avgResponseTime: 0, totalChecks: 0, failedChecks: 0 });
...
const loadStats = async () => {
  try {
    ...
    if (result) {
      stats.value = { uptime: parseFloat(result.uptime) || 0, avgResponseTime: parseFloat(result.avgResponseTime) || 0, totalChecks: result.totalChecks || 0, failedChecks: result.failedChecks || 0 };
    }
...
const loadAlerts = async () => { alerts.value = []; };
...
const handleDelete = () => {
  Modal.confirm({
    title: '确认删除',
    content: `确定要删除 API 监控 "${monitor.value?.name}" 吗？`,
    okText: '确认',
    cancelText: '取消',
    okType: 'danger',
    onOk: async () => { try { await monitorStore.deleteMonitor(id.value); message.success('删除成功'); router.push('/apis'); } catch (error: any) { message.error('删除失败：' + (error.message || '未知错误')); } },
  });
};
...
const getAlertColor = (severity: string) => {
  const colors: Record<string, string> = { INFO: '#1890ff', WARNING: '#faad14', CRITICAL: '#ff4d4f', FATAL: '#722ed1' };
  return colors[severity] || 'gray';
};

const getAlertBadgeStatus = (severity: string) => {
  const status: Record<string, 'success' | 'processing' | 'error' | 'warning' | 'default'> = { INFO: 'success', WARNING: 'processing', CRITICAL: 'error', FATAL: 'error' };
...
onMounted(() => { loadMonitor(); loadStats(); loadCheckResults(); loadAlerts(); });
</script>

<style scoped>
.api-detail-page { padding: 24px; }
.mt-4 { margin-top: 16px; }
.chart-container { height: 300px; }
.alert-item { margin-left: 16px; }
...
.alert-footer { display: flex; gap: 8px; }
</style>
