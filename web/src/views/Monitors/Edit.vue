    ...
    <a-card class="form-card">
      <a-form layout="vertical" :model="form" :rules="rules" ref="formRef">
        <a-form-item label="监控类型" name="type">
          <monitor-type-selector v-model="form.type" :disabled="true" />
          <span class="help-text">监控类型创建后不可更改</span>
        </a-form-item>
        ...
        <a-form-item>
          <a-space>
            <a-button type="primary" html-type="submit" :loading="submitting" @click="handleUpdate">
              保存修改
            </a-button>
...
import type { MonitorType, Monitor } from '@synthetic-monitoring/shared';
...
const loadMonitor = async () => {
  ...
  loading.value = true;
  try {
    const monitor = await monitorStore.getMonitorById(id);
    if (monitor) {
      form.type = monitor.type;
      form.name = monitor.name;
      form.target = monitor.target;
      form.interval = monitor.interval;
      form.timeout = monitor.timeout;
      form.regions = monitor.regions || [];
      form.config = (monitor as any).config || {};
      form.alertRules = (monitor.alertRules || []).map((r) => ({
        ...r,
        recipientsString: r.recipients?.join('\n') || '',
      }));
    }
  } catch (error: any) {
    console.error('Failed to load monitor:', error);
    message.error(error?.message || '加载监控任务失败');
    router.push('/monitors');
  } finally {
    loading.value = false;
  }
...
const handleUpdate = async () => {
  try {
    ...
    await monitorStore.updateMonitor(route.params.id as string, payload);
    message.success('监控任务更新成功');
    router.push('/monitors');
  } catch (error: any) {
    console.error('Failed to update monitor:', error);
    message.error(error?.message || '更新失败，请检查输入');
  } finally {
